...
**Corrected attribution (2026-09-22).** An earlier draft of this plan said the spec's §5 offered a self-checking rule — "pad 7 is the pad nearest the board edge (4.8 mm) and pad 1 is 7.5 mm inboard". **It does not, and never did.** That claim lived only in this project's memory file (`cn1-connector-pigtail-plan.md`), and it is wrong twice over: neither number appears in the geometry (pad 7 is 11.50 mm from the top edge and 3.32 mm from the curved right edge; pad 1 is 12.30 mm from the bottom edge and 3.49 mm from the curved edge), and the ordering is inverted — **pad 1 is the bottom-most pad, not pad 7**. The memory has been corrected.

What the spec's §5 actually said was weaker and, on its own terms, right: confirm the harness's blank position "against pad 7 on the first assembled board before power-on". Pad 7 is indeed the NC position. What it lacked was any statement of *where pad 7 physically sits*, which is what the copper resolves above. Task 6 adds that.
...
- `docs/superpowers/specs/2026-09-21-rev-b-connector-design.md` §5: the existing wording ("confirm the blank against pad 7") is correct but says nothing about where pad 7 physically is. Add the resolved geometry — pad 1 bottom-most at board-local y 19.00 up to pad 7 (NC) at y 11.50, all at x 59.50 — and the photo confirmation. Same addition to `phase0-measurements.md` P0.3/P0.6, which list the physical order as TODO.
- **Leave alone:** `phase0-measurements.md` (the P0 board-outline measurement) and `2026-08-03-tht-board-design.md` (the stock-envelope line) both give 63.3 × 31.3 as the **stock envelope**. That is still true — it is our board that shrank, not the can. Cite these by section, not line number; Task 6's own edits shift the line numbers underneath any such reference.
